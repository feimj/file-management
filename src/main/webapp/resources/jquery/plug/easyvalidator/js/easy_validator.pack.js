	...
	$("[reg],[url]:not([reg]),[tip]").hover(
		function(e) {
			if($(this).attr('tip') != undefined){
				...
				if($.browser.msie){
					$('body').append( '<p id="vtip"><span id="vtipArrow" style="width:21px; height:11px;display:inline-block;;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src=jquery/plug/easyvalidator/images/vtip_arrow.png, sizingMethod=scale);"></span>' + $(this).attr('tip') + '</p>' );
				}
				else{
					$('body').append( '<p id="vtip"><img id="vtipArrow" src="jquery/plug/easyvalidator/images/vtip_arrow.png"/>' + $(this).attr('tip') + '</p>' );
				}
...
function validate(obj){
	...
	//这里重新改过，当元素有disabled属性后，这个元素就不作验证
